...
To prepare the network initial filtering was needed based on three criterias:

1. **Remove artist which has no collaborations**
   - If an artist do not have any collaborations with other artists there is no meaningful way to use it in the analysis, these artists are therefore removed.
...
1. **Remove artist with too few song lyrics**
   - As a part of data retrieval the top 5 song lyrics (accordingly to the Spotify ratings in USA) were acquired from [Genius](https://genius.com/).
     However, for some artists all 5 song lyrics were not succesfully acquired due the song being less famous or pure instrumental.
...
<img src="../images/nw_genres.png" width="120%" />
...
This means that if for example the artist *Childish Gambino* has the genres *Rap* and *Hip-hop* an edge is created between the two genres.
As certain connections will occurr more often than others, a weight is included for the edges. This weight, will increase with the frequency of the connection.
The same is done for the nodes, if a genre appears more often the weight, and the size, of the node will increase.
